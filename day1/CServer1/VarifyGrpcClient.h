...
#include "Singleton.h"              // 单例模板：全局只保留一个客户端、一条 Channel
...
private:
    // 只允许经 GetInstance() 走一次，避免重复建 Channel
    VerifyGrpcClient() {
        // 目标地址格式 "ip:port"，要与 VarifyServer 的监听端口一致（见 config.ini 的 [VarifyServer] Port）
        // InsecureChannelCredentials = 明文传输、不加密；生产环境应换成 grpc::SslCredentials()
        std::shared_ptr<Channel> channel = grpc::CreateChannel("127.0.0.1:50051", grpc::InsecureChannelCredentials());
        // 由 Channel 造出 Stub。Stub 就是远端服务在本进程里的"代理"，方法签名与 proto 一一对应
        stub_ = VarifyService::NewStub(channel);
    }

    // Stub 的持有者：Stub 是抽象基类，NewStub 返回的是内部实现类
    // 用 unique_ptr 独占所有权，跟着单例一起活到进程结束
